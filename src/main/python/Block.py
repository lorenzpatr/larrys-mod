...
"""
def create_json_blockstates(file_path, file_name):
    data = {
        "variants": {
        "": {
            "model": f"larrys-mod:block/{file_name.split('.')[0]}",
        }}
    }
...
def create_json_models_block(file_path, file_name):
    data = {
        "parent": "minecraft:block/cube_all",
        "textures": {
            "all": f"larrys-mod:block/{file_name.split('.')[0]}",
        }
    }
...
def create_json_models_item(file_path, file_name):
    data = {
        "parent": f"larrys-mod:block/{file_name.split('.')[0]}",
    }
...
def check_if_named(file_path, technical_name):
    try:
        with open(file_path, "r", encoding="utf-8") as f:
            ...
            return f"block.larrys-mod.{technical_name}" in lang_data
...
def add_translation(file_path, item_name, display_name):
    ...
    key = f"block.larrys-mod.{item_name}"
...
class Block:
    def __init__(self, name, display_name, description):
        ...
        self.create_json_file_block("C:\\Users\\patri\\Downloads\\template-mod-template-1.21 (1)\\src\\main\\resources\\assets\\larrys-mod\\")

    def create_json_file_block(self, resource_path):
        lang = resource_path + "lang\\en_us.json"
        models_block = resource_path + "models/block/"
        models_item = resource_path + "models/item/"
        textures = resource_path + "textures/block/"
        blockstates = resource_path + "blockstates/"
        ...
        if os.path.isfile(models_block + file_name):
            print("Model:Block file exists")
        else:
            create_json_models_block(models_block + file_name, file_name)

        if os.path.isfile(models_item + file_name):
            print("Model:Item file exists")
        else:
            create_json_models_item(models_item + file_name, file_name)

        if os.path.isfile(blockstates + file_name):
            print("Blockstates file exists")
        else:
            create_json_blockstates(blockstates + file_name, file_name)
...
def main():
    ...
    solaris_block = Block("solaris_block", "Solaris Block", "")
